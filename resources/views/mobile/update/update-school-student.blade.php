    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-6 col-md-6 text-center">  
            ...
            <input type="text" name="institution" id="institution" 
                   class="form-control" 
                   placeholder="@lang('labels.text_name_school')" />
            </div>
...
<script> $(document).ready(function(){ $('#register').validate({ submitHandler: function(form) { $.post( "", { last_name : $("#lastname").val(), first_name : $("#firstname").val(), nationid : 0, gender : $("#gender").val(), birthdate : '', country : $("#country option:selected").text(), city : $("#city").val(), phone : '', studentid : '', speciality : '', institution_name : $("#institution").val() }, function(data, status) { window.location.href="{{ url('/web/board/'.$userid.'/'.$mode.'/'.$code.'/0') }}"; }); }, rules: { firstname: { required: true }, lastname: { required: true } }, messages: { firstname: { required: "Ingrese su nombre" }, lastname: { required: "Ingrese sus apellidos" } }, errorClass: "has-error", highlight: function(element, errorClass) { $(element.form).find("div[id=" + element.id + "-box]").addClass(errorClass); }, }); }); </script> 
