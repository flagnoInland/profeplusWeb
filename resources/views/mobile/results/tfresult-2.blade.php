...
<div class="container">
	...
	<div class="col-xs-12 col-sm-4 col-md-4 col-sm-offset-4 col-md-offset-4 text-center" id="report-box">
		<button id="report" class="btn btn-lg btn-green btn-block">
			RECIBIR<br/>INFORME
		</button>
	</div>
...
$(document).ready(function(){
    ...
    function drawChart(res){
		...
		if (res!= null){
			...
			ansyi = parseInt(res.ans_yesi);
			ansni = parseInt(res.ans_noi);
			ansdi = parseInt(res.ans_nai);
			totali = parseInt(res.inlessoni);
		}
		
		var maxw = parseInt(totali);
		if (total > totali){
			maxw = total;
		}
		...
		if (total != 0){
				vals[0]=Math.round(100*ansy/total)+"%";
				vals[1]=Math.round(100*ansn/total)+"%";
				vals[2]=Math.round(100*ansd/total)+"%";
				vals[3]=Math.round(100*ansnn/total)+"%";
		}
		
		var data = google.visualization.arrayToDataTable([
			["Answer", "Paso 1", { role: 'style' }, "Paso 2", {role:'style'} ],
			[0.5, ansyi, "#74b47c", ansy, "#daa520"],
			[1.5, ansni, "#74b47c", ansn, "#daa520"],
			[2.5, ansdi, "#74b47c", ansd, "#daa520"],
			[3.5, ansnni, "color: #74b47c", ansnn, "#daa520"],
		  ]);
		...
		var lbls = ["V","F","No Sabe","S.R."];
		var rects =  document.querySelectorAll('[text-anchor="middle"]');
		for (var i=0;i<rects.length-8;i+=2) { 
