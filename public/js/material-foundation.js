...
window.whatInput = function() {
    ...
    function eventBuffer() {
        clearTimer(), setInput(event), buffer = !0, timer = window.setTimeout(function() {
            buffer = !1;
        }, 650);
    }
    ...
    function unBufferedEvent(event) {
        clearTimer(), setInput(event);
    }
    function clearTimer() {
        window.clearTimeout(timer);
    }
    function setInput(event) {
        var eventKey = key(event), value = inputMap[event.type];
        if ("pointer" === value && (value = pointerType(event)), currentInput !== value) {
            var eventTarget = target(event), eventTargetNode = eventTarget.nodeName.toLowerCase(), eventTargetType = "input" === eventTargetNode ? eventTarget.getAttribute("type") : null;
            !body.hasAttribute("data-whatinput-formtyping") && currentInput && "keyboard" === value && "tab" !== keyMap[eventKey] && ("textarea" === eventTargetNode || "select" === eventTargetNode || "input" === eventTargetNode && nonTypingInputs.indexOf(eventTargetType) < 0) || ignoreMap.indexOf(eventKey) > -1 || switchInput(value);
        }
        "keyboard" === value && logKeys(eventKey);
    }
    function switchInput(string) {
        currentInput = string, body.setAttribute("data-whatinput", currentInput), inputTypes.indexOf(currentInput) === -1 && inputTypes.push(currentInput);
    }
    function key(event) {
    ...
    function logKeys(eventKey) {
        activeKeys.indexOf(keyMap[eventKey]) === -1 && keyMap[eventKey] && activeKeys.push(keyMap[eventKey]);
    }
    function unLogKeys(event) {
        var eventKey = key(event), arrayPos = activeKeys.indexOf(keyMap[eventKey]);
        arrayPos !== -1 && activeKeys.splice(arrayPos, 1);
    }
    function bindEvents() {
        body = document.body, window.PointerEvent ? (body.addEventListener("pointerdown", bufferedEvent), 
        body.addEventListener("pointermove", bufferedEvent)) : window.MSPointerEvent ? (body.addEventListener("MSPointerDown", bufferedEvent), 
        body.addEventListener("MSPointerMove", bufferedEvent)) : (body.addEventListener("mousedown", bufferedEvent), 
        body.addEventListener("mousemove", bufferedEvent), "ontouchstart" in window && body.addEventListener("touchstart", eventBuffer)), 
        body.addEventListener(mouseWheel, bufferedEvent), body.addEventListener("keydown", unBufferedEvent), 
        body.addEventListener("keyup", unBufferedEvent), document.addEventListener("keyup", unLogKeys);
    }
    function detectWheel() {
        return mouseWheel = "onwheel" in document.createElement("div") ? "wheel" : void 0 !== document.onmousewheel ? "mousewheel" : "DOMMouseScroll";
    }
    var body, activeKeys = [], buffer = !1, currentInput = null, nonTypingInputs = [ "button", "checkbox", "file", "image", "radio", "reset", "submit" ], mouseWheel = detectWheel(), ignoreMap = [ 16, 17, 18, 91, 93 ], inputMap = {
        keydown: "keyboard",
        keyup: "keyboard",
        mousedown: "mouse",
        mousemove: "mouse",
    ...
    return "addEventListener" in window && Array.prototype.indexOf && (document.body ? bindEvents() : document.addEventListener("DOMContentLoaded", bindEvents)), 
    {
        ask: function() {
            return currentInput;
        },
    ...
    function parseValue(str) {
        return !!/true/.test(str) || !/false/.test(str) && (isNaN(1 * str) ? str : parseFloat(str));
    }
    function hyphenate(str) {
        return str.replace(/([a-z])([A-Z])/g, "$1-$2").toLowerCase();
    }
    var FOUNDATION_VERSION = "6.2.3", Foundation = {
    ...
    function ImNotTouchingYou(element, parent, lrOnly, tbOnly) {
        ...
        return lrOnly ? left === right == !0 : tbOnly ? top === bottom == !0 : allDirs.indexOf(!1) === -1;
    }
    ...
    var keyCodes = {
        ...
        handleKey: function(event, component, functions) {
            ...
            if (cmds = "undefined" == typeof commandList.ltr ? commandList : Foundation.rtl() ? $.extend({}, commandList.ltr, commandList.rtl) : $.extend({}, commandList.rtl, commandList.ltr), 
            command = cmds[keyCode], fn = functions[command], fn && "function" == typeof fn) {
    ...
    function Move(duration, elem, fn) {
        function move(ts) {
            ...
            prog < duration ? anim = window.requestAnimationFrame(move, elem) : (window.cancelAnimationFrame(anim), 
    ...
    function Timer(elem, options, cb) {
        ...
        this.isPaused = !1, this.restart = function() {
            ...
            this.isPaused = !1, clearTimeout(timer), remain = remain <= 0 ? duration : remain, 
...
!function($) {
    var Abide = function() {
        ...
        return _createClass(Abide, [ {
            ...
            key: "findRadioLabels",
            value: function($els) {
                var _this3 = this, labels = $els.map(function(i, el) {
                    var id = el.id, $label = _this3.$element.find('label[for="' + id + '"]');
            ...
            value: function(groupName) {
                var $els = this.$element.find(':radio[name="' + groupName + '"]'), $labels = this.findRadioLabels($els), $formErrors = this.findFormError($els);
                $labels.length && $labels.removeClass(this.options.labelErrorClass), $formErrors.length && $formErrors.removeClass(this.options.formErrorClass), 
            ...
            value: function($el) {
                ...
                var goodToGo = [ clearRequire, validated, customValidator, equalTo ].indexOf(!1) === -1, message = (goodToGo ? "valid" : "invalid") + ".zf.abide";
            ...
            value: function($el, pattern) {
                ...
                var inputText = $el.val(), valid = !1;
                return inputText.length ? valid = this.options.patterns.hasOwnProperty(pattern) ? this.options.patterns[pattern].test(inputText) : pattern === $el.attr("type") || new RegExp(pattern).test(inputText) : $el.prop("required") || (valid = !0), 
                valid;
            }
        }, {
            key: "validateRadio",
            value: function(groupName) {
                var $group = this.$element.find(':radio[name="' + groupName + '"]'), valid = !1, required = !1;
            ...
            value: function() {
                ...
                $form.find("[data-abide-error]").css("display", "none"), $(":input", $form).not(":button, :submit, :reset, :hidden, :radio, :checkbox, [data-abide-ignore]").val("").removeAttr("data-invalid"), 
                $(":input:radio", $form).not("[data-abide-ignore]").prop("checked", !1).removeAttr("data-invalid"), 
                $(":input:checkbox", $form).not("[data-abide-ignore]").prop("checked", !1).removeAttr("data-invalid"), 
...
!function($) {
    var Accordion = function() {
        ...
        return _createClass(Accordion, [ {
            key: "_init",
            value: function() {
                this.$element.attr("role", "tablist"), this.$tabs = this.$element.children("li, [data-accordion-item]"), 
                this.$tabs.each(function(idx, el) {
                    var $el = $(el), $content = $el.children("[data-tab-content]"), id = $content[0].id || Foundation.GetYoDigits(6, "accordion"), linkId = el.id || id + "-label";
            ...
            value: function() {
                ...
                this.$element.find("li").each(function() {
                    ...
                    $elements.each(function(i) {
                        if ($(this).is($element)) return $prevElement = $elements.eq(Math.max(0, i - 1)).find("a").first(), 
                        $nextElement = $elements.eq(Math.min(i + 1, $elements.length - 1)).find("a").first(), 
                        $(this).children("[data-submenu]:visible").length && ($nextElement = $element.find("li:first-child").find("a").first()), 
                        $(this).is(":first-child") ? $prevElement = $element.parents("li").first().find("a").first() : $prevElement.children("[data-submenu]:visible").length && ($prevElement = $prevElement.find("li:last-child").find("a").first()), 
                        void ($(this).is(":last-child") && ($nextElement = $element.parents("li").first().next("li").find("a").first()));
                        ...
                        up: function() {
                            return $prevElement.attr("tabindex", -1).focus(), !0;
                        },
                        down: function() {
                            return $nextElement.attr("tabindex", -1).focus(), !0;
                        },
                        toggle: function() {
        ...
        return _createClass(Drilldown, [ {
            ...
            value: function() {
                ...
                this.$submenuAnchors.each(function() {
                    ...
                    $link.data("savedHref", $link.attr("href")).removeAttr("href"), $link.children("[data-submenu]").attr({
                        "aria-hidden": !0,
                        tabindex: 0,
                        role: "menu"
                    }), _this._events($link);
                ...
                }), this.$element.parent().hasClass("is-drilldown") || (this.$wrapper = $(this.options.wrapper).addClass("is-drilldown"), 
            ...
            value: function($elem) {
                ...
                $elem.off("click.zf.drilldown").on("click.zf.drilldown", function(e) {
                    ...
                    e.preventDefault()), _this._show($elem.parent("li")), _this.options.closeOnClick) {
                        var $body = $("body");
                        $body.off(".zf.drilldown").on("click.zf.drilldown", function(e) {
                            e.target === _this.$element[0] || $.contains(_this.$element[0], e.target) || (e.preventDefault(), 
                            _this._hideAll(), $body.off(".zf.drilldown"));
                        });
                    }
                });
            }
            ...
            value: function() {
                ...
                this.$menuItems.add(this.$element.find(".js-drilldown-back > a")).on("keydown.zf.drilldown", function(e) {
                    var $prevElement, $nextElement, $element = $(this), $elements = $element.parent("li").parent("ul").children("li").children("a");
                    $elements.each(function(i) {
                        if ($(this).is($element)) return $prevElement = $elements.eq(Math.max(0, i - 1)), 
                        void ($nextElement = $elements.eq(Math.min(i + 1, $elements.length - 1)));
                    }), Foundation.Keyboard.handleKey(e, "Drilldown", {
                        next: function() {
                            if ($element.is(_this.$submenuAnchors)) return _this._show($element.parent("li")), 
                            $element.parent("li").one(Foundation.transitionend($element), function() {
                                $element.parent("li").find("ul li a").filter(_this.$menuItems).first().focus();
                            }), !0;
                        },
                        previous: function() {
                            return _this._hide($element.parent("li").parent("ul")), $element.parent("li").parent("ul").one(Foundation.transitionend($element), function() {
                                setTimeout(function() {
                        ...
                        open: function() {
                            return $element.is(_this.$menuItems) ? $element.is(_this.$submenuAnchors) && (_this._show($element.parent("li")), 
                            $element.parent("li").one(Foundation.transitionend($element), function() {
                                $element.parent("li").find("ul li a").filter(_this.$menuItems).first().focus();
                            })) : (_this._hide($element.parent("li").parent("ul")), $element.parent("li").parent("ul").one(Foundation.transitionend($element), function() {
                                setTimeout(function() {
                                    $element.parent("li").parent("ul").parent("li").children("a").first().focus();
                                }, 1);
                            })), !0;
    ...
    var Dropdown = function() {
        ...
        return _createClass(Dropdown, [ {
            ...
            value: function() {
                ...
                for (this.$element.offset(Foundation.Box.GetOffsets(this.$element, this.$anchor, position, this.options.vOffset, this.options.hOffset)); !Foundation.Box.ImNotTouchingYou(this.$element, !1, !0) && this.counter; ) this._reposition(position), 
                this._setPosition();
            }
        }, {
            key: "_events",
...
!function($) {
    var DropdownMenu = function() {
        ...
        return _createClass(DropdownMenu, [ {
            ...
            key: "_events",
            value: function() {
                var _this = this, hasTouch = "ontouchstart" in window || "undefined" != typeof window.ontouchstart, parClass = "is-dropdown-submenu-parent", handleClickFn = function(e) {
                ...
                (this.options.clickOpen || hasTouch) && this.$menuItems.on("click.zf.dropdownmenu touchstart.zf.dropdownmenu", handleClickFn), 
                this.options.disableHover || this.$menuItems.on("mouseenter.zf.dropdownmenu", function(e) {
                    ...
                    isTab ? _this.$element.hasClass(_this.options.verticalClass) ? "left" === _this.options.alignment ? $.extend(functions, {
                        down: nextSibling,
                        up: prevSibling,
                        next: openSub,
                        previous: closeSub
...
!function($) {
    var Equalizer = function() {
        ...
        return _createClass(Equalizer, [ {
            ...
            key: "getHeights",
            value: function(cb) {
                for (var heights = [], i = 0, len = this.$watched.length; i < len; i++) this.$watched[i].style.height = "auto", 
            ...
            value: function(groups) {
                ...
                for (var i = 0, len = groups.length; i < len; i++) {
                    ...
                    if (groupsILength <= 2) $(groups[i][0][0]).css({
                        height: "auto"
                    }); else {
                        this.$element.trigger("preequalizedrow.zf.equalizer");
                        for (var j = 0, lenJ = groupsILength - 1; j < lenJ; j++) $(groups[i][j][0]).css({
                            height: max
                        });
...
!function($) {
    var Interchange = function() {
        ...
        return _createClass(Interchange, [ {
            ...
            key: "_addBreakpoints",
            value: function() {
                for (var i in Foundation.MediaQuery.queries) if (Foundation.MediaQuery.queries.hasOwnProperty(i)) {
            ...
            value: function(path) {
                if (this.currentPath !== path) {
                    ...
                    }).trigger(trigger) : path.match(/\.(gif|jpg|jpeg|png|svg|tiff)([?#].*)?/i) ? this.$element.css({
...
!function($) {
    var OffCanvas = function() {
        function OffCanvas(element, options) {
            ...
            this.$lastTrigger = $(), this.$triggers = $(), this._init(), this._events(), Foundation.registerPlugin(this, "OffCanvas");
        }
        return _createClass(OffCanvas, [ {
            key: "_init",
            value: function() {
                var id = this.$element.attr("id");
                if (this.$element.attr("aria-hidden", "true"), this.$triggers = $(document).find('[data-open="' + id + '"], [data-close="' + id + '"], [data-toggle="' + id + '"]').attr("aria-expanded", "false").attr("aria-controls", id), 
            ...
            value: function(event, trigger) {
                if (!this.$element.hasClass("is-open") && !this.isRevealed) {
                    ...
                    }), this.$triggers.attr("aria-expanded", "true"), this.$element.attr("aria-hidden", "false").trigger("opened.zf.offcanvas"), 
                    this.options.closeOnClick && this.$exiter.addClass("is-visible"), trigger && (this.$lastTrigger = trigger), 
            ...
            value: function(cb) {
                if (this.$element.hasClass("is-open") && !this.isRevealed) {
                    ...
                    this.options.closeOnClick && this.$exiter.removeClass("is-visible"), this.$triggers.attr("aria-expanded", "false"), 
                ...
                if (this.$slides.length > 1) {
                    if (this.options.swipe && this.$slides.off("swipeleft.zf.orbit swiperight.zf.orbit").on("swipeleft.zf.orbit", function(e) {
                        ...
                        $controls.attr("tabindex", 0).on("click.zf.orbit touchend.zf.orbit", function(e) {
                            e.preventDefault(), _this.changeSlide($(this).hasClass(_this.options.nextClass));
                        });
                    }
...
!function($) {
    var ResponsiveToggle = function() {
        ...
        return _createClass(ResponsiveToggle, [ {
            ...
            key: "_events",
            value: function() {
                this._updateMqHandler = this._update.bind(this), $(window).on("changed.zf.mediaquery", this._updateMqHandler), 
            ...
            value: function() {
                ...
                $(window).off("changed.zf.mediaquery", this._updateMqHandler), Foundation.unregisterPlugin(this);
            }
        } ]), ResponsiveToggle;
    }();
    ...
    var Reveal = function() {
        ...
        return _createClass(Reveal, [ {
            ...
            value: function() {
                ...
                }, this.isMobile = mobileSniff(), this.$anchor = $($('[data-open="' + this.id + '"]').length ? '[data-open="' + this.id + '"]' : '[data-toggle="' + this.id + '"]'), 
                this.$anchor.attr({
            ...
            key: "_updatePosition",
            value: function() {
                var left, top, width = this.$element.outerWidth(), outerWidth = $(window).width(), height = this.$element.outerHeight(), outerHeight = $(window).height();
                left = "auto" === this.options.hOffset ? parseInt((outerWidth - width) / 2, 10) : parseInt(this.options.hOffset, 10), 
                top = "auto" === this.options.vOffset ? height > outerHeight ? parseInt(Math.min(100, outerHeight / 10), 10) : parseInt((outerHeight - height) / 4, 10) : parseInt(this.options.vOffset, 10), 
                this.$element.css({
                    top: top + "px"
            ...
            value: function() {
                ...
                this.options.animationIn) {
                    ...
                    !function() {
                        ...
                        _this = _this3, _this3.options.overlay && Foundation.Motion.animateIn(_this3.$overlay, "fade-in"), 
                        Foundation.Motion.animateIn(_this3.$element, _this3.options.animationIn, function() {
                            _this3.focusableElements = Foundation.Keyboard.findFocusable(_this3.$element), afterAnimationFocus();
                        });
                    }();
                ...
                this.$element.attr({
                    "aria-hidden": !1,
                    tabindex: -1
                }).focus(), this.$element.trigger("open.zf.reveal"), this.isMobile ? (this.originalScrollPos = window.pageYOffset, 
                $("html, body").addClass("is-reveal-open")) : $("body").addClass("is-reveal-open"), 
                setTimeout(function() {
            ...
            value: function() {
                ...
                this.focusableElements = Foundation.Keyboard.findFocusable(this.$element), this.options.overlay || !this.options.closeOnClick || this.options.fullScreen || $("body").on("click.zf.reveal", function(e) {
                    ...
                    Foundation.Keyboard.handleKey(e, "Reveal", {
                        tab_forward: function() {
                            return _this.$element.find(":focus").is(_this.focusableElements.eq(-1)) ? (_this.focusableElements.eq(0).focus(), 
                            !0) : 0 === _this.focusableElements.length || void 0;
                        },
                        tab_backward: function() {
                            return _this.$element.find(":focus").is(_this.focusableElements.eq(0)) || _this.$element.is(":focus") ? (_this.focusableElements.eq(-1).focus(), 
                            !0) : 0 === _this.focusableElements.length || void 0;
                        },
                        open: function() {
            ...
            value: function() {
                function finishUp() {
                    _this.isMobile ? ($("html, body").removeClass("is-reveal-open"), _this.originalScrollPos && ($("body").scrollTop(_this.originalScrollPos), 
                    _this.originalScrollPos = null)) : $("body").removeClass("is-reveal-open"), _this.$element.attr("aria-hidden", !0), 
            ...
            key: "destroy",
            value: function() {
                this.options.overlay && (this.$element.appendTo($("body")), this.$overlay.hide().off().remove()), 
                this.$element.hide().off(), this.$anchor.off(".zf"), $(window).off(".zf.reveal:" + this.id), 
                Foundation.unregisterPlugin(this);
            }
        } ]), Reveal;
    }();
        ...
        return _createClass(Slider, [ {
            ...
            value: function(e, $handle, val) {
                ...
                if (val) value = this._adjustValue(null, val), hasVal = !0; else {
                    e.preventDefault();
                    var _this = this, vertical = this.options.vertical, param = vertical ? "height" : "width", direction = vertical ? "top" : "left", eventOffset = vertical ? e.pageY : e.pageX, barDim = (this.$handle[0].getBoundingClientRect()[param] / 2, 
                    this.$element[0].getBoundingClientRect()[param]), windowScroll = vertical ? $(window).scrollTop() : $(window).scrollLeft(), elemOffset = this.$element.offset()[direction];
                    e.clientY === e.pageY && (eventOffset += windowScroll);
                    var barXY, eventFromBar = eventOffset - elemOffset;
                    if (barXY = eventFromBar < 0 ? 0 : eventFromBar > barDim ? barDim : eventFromBar, 
                    offsetPct = percent(barXY, barDim), value = (this.options.end - this.options.start) * offsetPct + this.options.start, 
                    Foundation.rtl() && !this.options.vertical && (value = this.options.end - value), 
            ...
            value: function($handle) {
                ...
                if (this.inputs.off("change.zf.slider").on("change.zf.slider", function(e) {
                    ...
                    return !_this.$element.data("dragging") && void ($(e.target).is("[data-slider-handle]") || (_this.options.doubleSided ? _this._handleEvent(e) : _this._handleEvent(e, _this.$handle)));
...
!function($) {
    ...
    var Sticky = function() {
        ...
        return _createClass(Sticky, [ {
            ...
            key: "_parsePoints",
            value: function() {
                for (var top = "" == this.options.topAnchor ? 1 : this.options.topAnchor, btm = "" == this.options.btmAnchor ? document.documentElement.scrollHeight : this.options.btmAnchor, pts = [ top, btm ], breaks = {}, i = 0, len = pts.length; i < len && pts[i]; i++) {
                    var pt;
            ...
            key: "_setSticky",
            value: function() {
                var _this = this, stickTo = this.options.stickTo, mrgn = "top" === stickTo ? "marginTop" : "marginBottom", notStuckTo = "top" === stickTo ? "bottom" : "top", css = {};
            ...
            value: function(isTop) {
                var stickTo = this.options.stickTo, stickToTop = "top" === stickTo, css = {}, anchorPt = (this.points ? this.points[1] - this.points[0] : this.anchorHeight) - this.elemHeight, mrgn = stickToTop ? "marginTop" : "marginBottom", topOrBottom = isTop ? "top" : "bottom";
                css[mrgn] = 0, css.bottom = "auto", isTop ? css.top = 0 : css.top = anchorPt, css.left = "", 
            ...
            value: function(cb) {
                ...
                "none" == this.$element.css("display") && (newContainerHeight = 0), this.containerHeight = newContainerHeight, 
                this.$container.css({
            ...
            value: function() {
                ...
                $(window).off(this.scrollListener), this.wasWrapped ? this.$element.unwrap() : this.$container.removeClass(this.options.containerClass).css({
                    height: ""
                }), Foundation.unregisterPlugin(this);
            }
        } ]), Sticky;
    ...
    var Tabs = function() {
        ...
        return _createClass(Tabs, [ {
            ...
            value: function() {
                ...
                this.$tabTitles.off("keydown.zf.tabs").on("keydown.zf.tabs", function(e) {
                    if (9 !== e.which) {
                        ...
                        $elements.each(function(i) {
                            if ($(this).is($element)) return void (_this.options.wrapOnKeys ? ($prevElement = 0 === i ? $elements.last() : $elements.eq(i - 1), 
            ...
            value: function() {
                ...
                this.options.matchHeight && null != this._setHeightMqHandler && $(window).off("changed.zf.mediaquery", this._setHeightMqHandler), 
                Foundation.unregisterPlugin(this);
            }
        } ]), Tabs;
    }();
        ...
        return _createClass(Toggler, [ {
            ...
            value: function() {
                ...
                this.$element.is(":hidden") ? Foundation.Motion.animateIn(this.$element, this.animationIn, function() {
                    _this._updateARIA(!0), this.trigger("on.zf.toggler");
                }) : Foundation.Motion.animateOut(this.$element, this.animationOut, function() {
                    _this._updateARIA(!1), this.trigger("off.zf.toggler");
                });
            }
        }, {
            key: "_updateARIA",
    ...
    var Tooltip = function() {
        ...
        return _createClass(Tooltip, [ {
            ...
            value: function() {
                ...
                this.options.tipText = this.options.tipText || this.$element.attr("title"), this.template = this.options.template ? $(this.options.template) : this._buildTemplate(elemId), 
            ...
            value: function() {
                ...
                this.options.disableHover || this.$element.on("mouseenter.zf.tooltip", function(e) {
                    ...
                    return isFocus = !0, _this.isClick ? (_this.options.clickOpen || (isFocus = !1), 
                    !1) : void _this.show();
