...
<script> $( function() { var birth = $( "#birthdate" ).datepicker({format: 'dd/mm/yyyy'}); var mdate = '{{ $user['birthdate'] }}'; var fdate = mdate.substr(8,2)+'/'+mdate.substr(5,2)+'/'+mdate.substr(0,4); $("#birthdate").val(fdate); $( "#birthdate" ).datepicker('setValue',fdate); var gen = '{{ $user['gender'] }}'; if (gen == 'Female' || gen == 'Femenino'){ $("select option[value='Female']").attr("selected","selected"); } else { $("select option[value='Male']").attr("selected","selected"); } $("#back-icon").attr("href", "{{ url('/web/board/'.$userid.'/'.$mode.'/'.$code.'/1') }}"); $("#exit-icon").attr("href", "{{ url('/web/board/'.$userid.'/'.$mode.'/'.$code.'/1') }}"); } ); </script> 
...
<script> $(document).ready(function(){ $('#register').validate({ submitHandler: function(form) { $.post( "", { last_name : $("#lastname").val(), first_name : $("#firstname").val(), nationid : $("#dni").val(), gender : $("#gender").val(), birthdate : $("#birthdate").val(), country : $("#country option:selected").text(), city : $("#city").val(), phone : $("#phone").val(), studentid : '', speciality : '', institution_name : '' }, function(data, status) { window.location.href="{{ url('/web/board/'.$userid.'/'.$mode.'/'.$code.'/1') }}"; }); }, rules: { firstname: { required: true }, lastname: { required: true }, birthdate: { required: true }, dni: { required: true } }, messages: { firstname: { required: "Ingrese su nombre" }, lastname: { required: "Ingrese sus apellidos" }, dni: { required: "Ingrese su número de D.N.I." }, birthdate: { required: "Ingrese su fecha de nacimiento" } }, errorClass: "has-error", highlight: function(element, errorClass) { $(element.form).find("div[id=" + element.id + "-box]").addClass(errorClass); }, }); }); </script> 
