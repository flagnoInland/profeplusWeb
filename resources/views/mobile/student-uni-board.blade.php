...
<div class="container ">
  ...
  <div class="col-xs-12 col-sm-12 col-md-12 text-center">
    ...
    <div class="form-group-lg text-center" id="code-box">
      ...
      <br>
      <div class="col-xs-12 col-sm-12 col-md-12 text-center">
        <a href="{{ url('/web/student-manual-list/'.$userid.'/1/0000') }}" class="btn btn-lg-2 btn-base btn-block">
        @lang('labels.text_use_manual')
        </a>
      </div>

      <!--div class="col-xs-12 col-sm-12 col-md-12 text-center">
        <a href="{{ url('/web/exam/student/'.$userid.'/1') }}" class="btn btn-lg-2 btn-orange btn-block">
...
<div class="text-center footer">
  <span>¡ELEVA TUS COMPETENCIAS Y </span><span>EMPRENDE TU PASIÓN!</span><br/>
  Visita: www.E-QUIPU.pe
</div>
    ...
    <div class="modal-wrapper">
      <div class="modal-content modal-body">
        ...
        <div class="text-center">
          <br/>
          Código&nbsp;equivocado&nbsp;o<br/>su&nbsp;lección&nbsp;ha&nbsp;finalizado.
          <br/><br/>
        </div>
        <div class="text-center">
...
$(document).ready(function(){
    ...
    $("#info-icon").attr("href", "{{ url('/web/info-list/'.$userid.'/1/0/0000') }}");
    $("#mobile-icon").attr("href", "{{ url('/web/manual/mobile/'.$userid.'/1/0/0000') }}");
    $("#mode-icon").attr("href", "{{ url('/web/user/'.$userid) }}");
    $("#profile-icon").attr("href", "{{ url('/web/update/'.$userid.'/1/0/0000') }}");
    
    $("#exit-icon").click(function(){
      window.location.replace("{{ url('/') }}");
    });
    $("#back-icon").click(function(){
      window.location.replace("{{ url('/web/login/1/0') }}");
    });
    var studentData = {
      'id' : {{$userid}},
      'share': 0
    ...
    function takeQuestion(){
      $.ajax({ 
        type: "GET", 
        url: "{{url('/web/student/')}}"+'/'+{{$userid}}+'/'+{{$userid}}+'/'+accesscode+'/Z/1', 
        dataType: 'json', 
        success: function(data, status) { 
          ...
          if (type == 1 || type == 2){
            ...
            var forQtn = '/' +type+'/'+survey+'/'+check+'/1';
            window.location.href = "{{url('/web/answer-board/')}}" + forUserLesson + forQtn;
          }
        }, 
        statusCode: { 
          404: function() { 
